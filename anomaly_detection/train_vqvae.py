import argparse
# 尝试导入comet_ml模块，如果不存在则创建模拟对象
try:
    import comet_ml
    HAS_COMET_ML = True
    print("[INFO] comet_ml模块已成功导入")
except ImportError:
    HAS_COMET_ML = False
    print("[警告] comet_ml模块不存在，使用模拟对象代替")
    ...
    class MockCometML:
        ...
        def Experiment(self):
            return type('MockExperiment', (), {
                'set_name': lambda self, name: None,
                'add_tag': lambda self, tag: None,
                'log_parameters': lambda self, params: None,
                'log_metric': lambda self, name, value: None,
...
def main(device, config, save_dir, logger, data_init_loc, args):
    ...
    if os.path.exists(os.path.join(save_dir, 'checkpoints')):
        print('Checkpoint Directory Already Exists - automatically continuing (will overwrite files inside)')
    else:
        os.makedirs(os.path.join(save_dir, 'checkpoints'))

    # 只有当logger不为None时才尝试记录参数
    if logger is not None:
        logger.log_parameters(config)
    else:
        print("[INFO] 未使用日志记录器，跳过参数记录")
    ...
    if os.path.exists(os.path.join(save_dir, 'configs')):
        print('Saved Config Directory Already Exists - automatically continuing (will overwrite files inside)')
    else:
...
if __name__ == '__main__':
    ...
    if args.comet_log:
        ...
        try:
            # 检查config中是否有comet_config配置
            comet_logger = comet_ml.Experiment(
                api_key=config.get('comet_config', {}).get('api_key', ''),
                project_name=config.get('comet_config', {}).get('project_name', ''),
                workspace=config.get('comet_config', {}).get('workspace', ''),
            )
            comet_logger.add_tag(args.comet_tag)
            comet_logger.set_name(args.comet_name)
        except Exception as e:
            print(f"[警告] 创建comet_ml实验失败: {e}")
            comet_logger = None
    else:
        print('[INFO] 未启用comet日志记录')
        comet_logger = None

    # (7) Set up GPU / CPU - 支持Mac的MPS后端
    if torch.backends.mps.is_available():
        device = 'mps'
        print("使用MPS后端进行计算")
    elif torch.cuda.is_available() and args.model_init_num_gpus >= 0:
