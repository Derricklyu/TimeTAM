...
# 定义数据集路径
dataset_dir = '/Users/derrick_lyu/Desktop/课题研究/实验/TOTEM-main/datasets/MSL'

# 创建结果保存目录
result_dir = '/Users/derrick_lyu/Desktop/课题研究/实验/TOTEM-main/msl_analysis_results_html'
if not os.path.exists(result_dir):
    os.makedirs(result_dir)

print("开始分析MSL数据集并生成HTML报告...")

# 1. 读取数据集
train_data = np.load(os.path.join(dataset_dir, 'MSL_train.npy'), allow_pickle=True)
test_data = np.load(os.path.join(dataset_dir, 'MSL_test.npy'), allow_pickle=True)
test_labels = np.load(os.path.join(dataset_dir, 'MSL_test_label.npy'), allow_pickle=True)
...
sample_df.to_csv(os.path.join(result_dir, 'msl_sample.csv'), index=False, encoding='utf-8-sig')
...
stats_df.to_csv(os.path.join(result_dir, 'msl_statistics_full.csv'), encoding='utf-8-sig')
...
# 确定正常和异常标签
normal_label = 0 if 0 in label_counts else False
anomaly_label = 1 if 1 in label_counts else True

labels = ['正常样本', '异常样本']
sizes = [label_counts[normal_label], label_counts[anomaly_label]]
colors = ['#66b3ff', '#ff6666']
explode = (0.1, 0)
plt.pie(sizes, explode=explode, labels=labels, colors=colors,
        autopct='%1.1f%%', shadow=True, startangle=90)
plt.axis('equal')  # 保证饼图是圆的
plt.title('MSL测试集标签分布')
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'msl_label_distribution_pie.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'msl_label_distribution_bar.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'msl_feature_distribution.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'msl_time_series.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('MSL训练数据特征相关性热力图')
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'msl_feature_correlation.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('MSL训练数据特征箱线图')
plt.xlabel('特征')
plt.ylabel('特征值')
plt.xticks(rotation=45)
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig(os.path.join(result_dir, 'msl_feature_boxplot.png'), dpi=300, bbox_inches='tight')
plt.close()
...
fig.update_layout(height=800, width=1200, title_text="MSL测试数据时间序列（前1000步）")
fig.write_html(os.path.join(result_dir, 'msl_interactive_time_series.html'))
...
fig.write_html(os.path.join(result_dir, 'msl_interactive_correlation.html'))
...
<body>
    ...
    <div class="section">
        <h2>1. 数据集概述</h2>
        <p>MSL (Mars Science Laboratory) 数据集是由美国国家航空航天局(NASA)提供的用于异常检测的经典数据集，包含火星科学实验室任务中的传感器数据。</p>
        <div class="highlight">
            ...
            <ul>
                ...
                <li><strong>异常样本比例:</strong> {label_counts[anomaly_label]/len(test_labels.flatten())*100:.2f}%</li>
            </ul>
        </div>
    </div>
    ...
    <div class="section">
        <h2>4. 标签分布分析</h2>
        <p>MSL数据集的标签分布呈现明显的不平衡特性，正常样本占大多数，而异常样本相对较少。</p>
        
        <div class="image-container">
            <img src="msl_label_distribution_pie.png" alt="MSL标签分布饼图">
            <div class="caption">图1: MSL测试集标签分布饼图</div>
        </div>
        
        <div class="image-container">
            <img src="msl_label_distribution_bar.png" alt="MSL标签分布条形图">
            <div class="caption">图2: MSL测试集标签分布条形图</div>
        </div>
        
        <div class="highlight">
            <p><strong>标签分布详情:</strong></p>
            <ul>
                <li>正常样本: {label_counts[normal_label]} 个，占比 {label_counts[normal_label]/len(test_labels.flatten())*100:.2f}%</li>
                <li>异常样本: {label_counts[anomaly_label]} 个，占比 {label_counts[anomaly_label]/len(test_labels.flatten())*100:.2f}%</li>
            </ul>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="image-container">
            <img src="msl_feature_distribution.png" alt="MSL特征值分布直方图">
            <div class="caption">图3: MSL特征值分布直方图（前12个特征）</div>
        </div>
        
        <p>从直方图可以看出，大部分特征的值分布情况，这有助于我们了解数据的基本特性和可能需要的预处理步骤。</p>
    </div>
    
    <div class="section">
        <h2>6. 时间序列分析</h2>
        <p>MSL数据集是一个时间序列数据集，以下是部分特征的时间序列示例：</p>
        
        <div class="image-container">
            <img src="msl_time_series.png" alt="MSL时间序列图">
            <div class="caption">图4: MSL测试数据时间序列图（前5个特征的前500个时间步）</div>
        </div>
        ...
        <div class="interactive-chart">
            ...
            <a href="msl_interactive_time_series.html" target="_blank">MSL交互式时间序列图</a>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="image-container">
            <img src="msl_feature_correlation.png" alt="MSL特征相关性热力图">
            <div class="caption">图5: MSL特征相关性热力图</div>
        </div>
        ...
        <div class="interactive-chart">
            ...
            <a href="msl_interactive_correlation.html" target="_blank">MSL交互式特征相关性热力图</a>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="image-container">
            <img src="msl_feature_boxplot.png" alt="MSL特征箱线图">
            <div class="caption">图6: MSL特征箱线图（前12个特征）</div>
        </div>
        
        <p>从箱线图可以看出，各特征的值分布情况以及可能存在的异常值。</p>
    </div>
    ...
    <div class="section">
        ...
        <div class="conclusion">
            <h3>主要发现</h3>
            <ol>
                <li><strong>数据标准化:</strong> 观察统计特征，MSL数据集可能已经过一定的标准化处理</li>
                <li><strong>类别不平衡:</strong> 异常样本仅占{label_counts[anomaly_label]/len(test_labels.flatten())*100:.2f}%，正常样本占{label_counts[normal_label]/len(test_labels.flatten())*100:.2f}%，属于典型的不平衡数据集</li>
                <li><strong>特征分布:</strong> 各特征呈现不同的分布特性，有的集中在某个范围内，有的分布较为分散</li>
                <li><strong>特征相关性:</strong> 部分特征之间存在较强的相关性，这可能反映了系统的内部联系</li>
                <li><strong>多维时间序列:</strong> 数据为{feature_dim}维时间序列，适合用于时间序列异常检测任务</li>
            </ol>
        </div>
    ...
    <div class="section">
        <h2>10. 结论</h2>
        <p>MSL数据集是一个高质量的异常检测基准数据集，具有典型的时间序列特性和类别不平衡特点。通过本报告的详细分析，我们对数据集的结构、分布和特点有了全面的了解，为后续的异常检测实验提供了重要的参考依据。</p>
        <p>建议在后续实验中充分考虑数据集的不平衡性和时间序列特性，选择合适的模型和评价指标，以获得更好的检测效果。</p>
    </div>
    
    <footer>
        <p style="text-align: center; color: #666; margin-top: 50px;">MSL数据集分析报告 © {time.strftime('%Y')}</p>
    </footer>
</body>
</html>
"""
...
# 保存HTML报告
with open(os.path.join(result_dir, 'msl_analysis_report.html'), 'w', encoding='utf-8') as f:
    f.write(report_html)

print(f"\nMSL数据集分析完成！")
print(f"HTML报告和所有图表已保存至：{result_dir}")
print(f"请打开 {os.path.join(result_dir, 'msl_analysis_report.html')} 查看详细分析结果。")

# 自动打开HTML报告
webbrowser.open('file://' + os.path.join(result_dir, 'msl_analysis_report.html'))